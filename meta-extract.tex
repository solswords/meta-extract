...
can be reduced to {\tt (car $x$)}.
(This is an unnecessarily strong check: it means ACL2's determination
...
since {\tt typespec-check} is one of its known functions, the
...
\noindent which, when expanded, implies that the evaluation of {\tt
  (cadr term)} may either be {\tt T}, {\tt NIL}, or a non-Boolean
symbol.  These three easy cases are then sufficient to prove {\tt nth-symbolp-meta}.
...
utility, such as {\tt mfc-rw+}, {\tt meta-extract-formula}, etc.  The
...
We now describe the allowed {\tt obj} arguments to {\tt
  meta-extract-global-fact} and the terms they produce:
...
\item {\tt (:formula $f$)} produces the value of {\tt
  ...
  \item the defchoose axiom of $f$ if it is a defchoose function.
  \end{itemize}
...
\item {\tt (:lemma $f$ $n$)} produces a term corresponding to the
  ...
  the length of the indicated list).  Here {\tt rewrite-rule-term}
...
\item {\tt (:fncall $f$ $L$)} produces a term {\tt (equal $c$ '$v$)}, where
    $c$ is the application of $f$ to the quotations of the values in argument list $L$, and $v$ is
    the value of that call as evaluated by
    {\tt magic-ev-fncall}.  This allows metafunctions to assume that {\tt magic-ev-fncall}
...
\item {\tt (:typeset $\var{term}$)} produces a term stating the correctness of
    the typeset produced by {\tt mfc-ts} for $\var{term}$.  Specifically, it produces the term
